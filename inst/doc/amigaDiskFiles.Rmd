...
from [AmigaForever](www.amigaforever.com). The same is true for much of the
...
* put files or directories onto a virtual disk;
...
need to know a bit more about the Amiga's file system and they way information
...
already booted when the disk is insert, the bootblock will just be used to
...
The bitmap block, contains information on which blocks on the disk are in use
and which are free. The first for bytes in this block form a checksum, the
