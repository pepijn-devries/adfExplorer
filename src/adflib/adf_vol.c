                              ...
                              const uint8_t * const    code )
{
    ...
    if ( vol->dev->devType != DEVTYPE_FLOPDD &&
         ...
         vol->dev->devType != DEVTYPE_HARDFILE) /*Edit PdV*/
    {
        return RC_ERROR;
    }
    ...
    // boot.rootBlock = 880; /* Edit PdV */
    boot.rootBlock = (vol->lastBlock + 1 - vol->firstBlock)/2;  /* Edit PdV */
...
/*    struct bDirCacheBlock dirc;*/
    ...
    if (!vol) { 
        //(*adfEnv.eFct)("adfCreateVol : malloc vol"); ?* Edit PdV */
        return NULL;
    }
    vol->dev = dev;
...
*/
    ...
    if (!vol->volName) { 
        //(*adfEnv.eFct)("adfCreateVol : malloc"); /* Edit PdV */
